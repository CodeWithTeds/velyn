...
export function Food4Editor({
  ...
  const [mainTitle, setMainTitle] = useState('CRISPY\nSLICE');
  const [subtitle, setSubtitle] = useState('HANDCRAFTED');
  const [badgeText, setBadgeText] = useState('NOW ONLY');
  const [price, setPrice] = useState('₱249');
  const [footerText, setFooterText] = useState('VISIT US AT\n@SKETCHY_BITES');

  const [primaryColor, setPrimaryColor] = useState('#E9C46A');
  const [secondaryColor, setSecondaryColor] = useState('#2A9D8F');
  const [accentColor, setAccentColor] = useState('#E76F51');

  const primarySwatches = ['#E9C46A', '#FFD166', '#F4A261', '#E76F51', '#264653', '#FFFFFF'];
  const secondarySwatches = ['#2A9D8F', '#118AB2', '#073B4C', '#264653', '#F4A261', '#E76F51'];
  const accentSwatches = ['#E76F51', '#EF476F', '#FFD166', '#06D6A0', '#118AB2', '#000000'];
  ...
  return (
    <div className={`grid h-full gap-4 bg-[#f5f5f7] p-4 md:grid-cols-[320px_minmax(0,1fr)] md:grid-rows-[auto_minmax(0,1fr)] md:p-6 ${fullscreen ? 'min-h-0' : 'max-h-[82vh] min-h-[620px]'}`}>
      <EditorToolbar
        description="Adjust text, rough graphics, and colors for this sketchy poster."
        label="Food Layer"
      ...
      <InspectorPanel eyebrow="Food 4" title="Sketchy Bites" footer="1080 x 1920 px - 9:16 Story format" description="Adjust text and image layout">
        ...
        <section className="space-y-4 rounded-md border border-slate-200 bg-slate-50 p-4">
          ...
          <div>
            <div className="mb-2 flex items-center justify-between gap-3">
              <span className="text-xs font-bold uppercase tracking-widest text-slate-500">Primary / Splash</span>
          ...
          <div className="pt-2">
            <div className="mb-2 flex items-center justify-between gap-3">
              <span className="text-xs font-bold uppercase tracking-widest text-slate-500">Background Color</span>
          ...
          <div className="pt-2">
            <div className="mb-2 flex items-center justify-between gap-3">
              <span className="text-xs font-bold uppercase tracking-widest text-slate-500">Accent / Details</span>
        ...
        <section className="mt-8">
          <DownloadButton targetRef={downloadRef} fileName="sketchy-bites-poster.png" className="w-full" />
        </section>
      </InspectorPanel>
    </div>
  );
