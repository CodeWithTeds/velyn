...
type Template2EditorProps = {
  defaultImage?: string;
  fullscreen?: boolean;
};

export function Template2Editor({
  defaultImage = '/images/test1.png',
  fullscreen = false,
  ...
  return (
    ...
    >
      <EditorToolbar
        description="Drag the image, resize with zoom, then tune contrast for a striking brutalist duotone."
        label="Acid Poster"
      ...
      >
          <section className="rounded-md border border-slate-200 bg-slate-50 p-4">
            ...
            <p className="mt-2 text-sm text-slate-600">Portrait source, overlaid with neon yellow multiply.</p>
          ...
          <section className="space-y-5">
            <div className="flex items-center justify-between">
              ...
              <button onClick={resetTransform} className="text-xs font-bold uppercase tracking-widest text-slate-400 hover:text-black">Reset</button>
            </div>
            <EditorSlider label="Zoom" min={60} max={240} value={imageTransform.scale} suffix="%" onChange={(value) => updateTransform('scale', value)} />
          ...
          <section className="space-y-5">
            ...
            <EditorSlider label="Brightness" min={55} max={140} value={imageTransform.brightness} suffix="%" onChange={(value) => updateTransform('brightness', value)} />
            <EditorSlider label="Contrast" min={80} max={250} value={imageTransform.contrast} suffix="%" onChange={(value) => updateTransform('contrast', value)} />
          </section>
          ...
          <section className="mt-8">
            <DownloadButton targetRef={downloadRef} fileName="template2-acid-download.png" className="w-full" />
          </section>
      </InspectorPanel>
    </div>
  );
