...
import { Template3Poster } from './Template3Poster';

type Template3EditorProps = {
  defaultImage?: string;
  fullscreen?: boolean;
};

export function Template3Editor({
  defaultImage = '/images/developer/image.png',
  fullscreen = false,
}: Template3EditorProps) {
  ...
  return (
    ...
    >
      <EditorToolbar
        description="Drag the figure, resize with zoom, then tune tone for a clean blue editorial poster."
        label="City Poster"
      ...
      <InspectorPanel
        description="Drag the poster to reposition the figure, or use exact layer controls below."
        eyebrow="Template 3"
      ...
      >
        <section className="rounded-md border border-slate-200 bg-slate-50 p-4">
          ...
          <p className="mt-2 text-sm text-slate-600">Cutout subject layered over locked blue editorial type.</p>

          <label className="mt-4 inline-flex w-full cursor-pointer items-center justify-center rounded-md bg-slate-950 px-5 py-3 text-xs font-bold uppercase tracking-widest text-white transition hover:bg-[#0753aa]">
            Change picture
        ...
        <section className="space-y-5">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="text-xs font-bold uppercase tracking-widest text-slate-400 hover:text-[#0753aa]"
            >
              Reset
            </button>
          </div>
        ...
        <section className="mt-8">
          <DownloadButton targetRef={downloadRef} fileName="template3-city-orbit-download.png" className="w-full" />
        </section>
      </InspectorPanel>
    </div>
  );
