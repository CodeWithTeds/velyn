...
import { Template6Poster } from './Template6Poster';

type Template6EditorProps = {
  defaultImage?: string;
  fullscreen?: boolean;
};

export function Template6Editor({
  defaultImage = '/images/test3.png',
  fullscreen = false,
}: Template6EditorProps) {
  ...
  return (
    ...
    >
      <EditorToolbar
        description="Position the photo within the hero frame. Text is kept in the margins for a spacious feel."
        label="Gallery"
      ...
      <InspectorPanel
        description="Fine-tune the subject within the gallery frame. Bottom area is reserved for typography."
        eyebrow="Template 6"
        footer="1080 x 1920 px - 9:16 Gallery Portrait"
        title="Nostalgic Gallery"
      >
        <section className="rounded-md border border-slate-200 bg-slate-50 p-4">
          <p className="text-xs font-bold uppercase tracking-widest text-slate-500">Image Source</p>
          <p className="mt-2 text-sm text-slate-600">The photo will be layered with nostalgic grain and classroom notes.</p>

          <label className="mt-4 inline-flex w-full cursor-pointer items-center justify-center rounded-md bg-slate-950 px-5 py-3 text-xs font-bold uppercase tracking-widest text-white transition hover:bg-teal-800">
            Change picture
        ...
        <section className="mt-8">
          <DownloadButton targetRef={downloadRef} fileName="template6-thinking-download.png" className="w-full" />
        </section>
      </InspectorPanel>
    </div>
  );
